...
function cart(applied: boolean) {
  return {
    ...
    couponCode: applied ? "BIOREZA20" : null,
...
async function installApi(page: Page) {
  ...
  await page.route("**/api/v1/**", async (route) => {
    ...
    if (path.endsWith("/cart/coupon") && request.method() === "POST") {
      applyRequests += 1;
      expect(request.postDataJSON()).toEqual({ code: "BIOREZA20" });
...
test("BIOREZA20 survives cart refresh and checkout, then is revalidated at order placement", async ({
  page,
}) => {
  ...
  await input.fill("  bioreza20 ");
  await page.getByRole("button", { name: "Apply" }).dblclick();
  await expect(page.getByText("BIOREZA20", { exact: false }).first()).toBeVisible();
  ...
  await page.goto("/cart", { waitUntil: "networkidle" });
  await page.getByLabel("Promo code").fill("BIOREZA20");
  ...
  const reviewPromo = page.locator(".sf-checkout-review-promo");
  await expect(reviewPromo).toContainText("BIOREZA20");
